      ...
      <nav className="flex-1 p-4 space-y-1">
        ...
        >
          <div className="indicator">
            <BellIcon className={`size-5 text-base-content opacity-70 ${hasNewNotifications ? 'animate-pulse' : ''}`} />
            {notificationCount > 0 && (
              <span className="badge badge-sm badge-error indicator-item">{notificationCount}</span>
            )}
          </div>
        ...
        <Link
          to="/groups"
          className={`btn btn-ghost justify-start w-full gap-3 px-3 normal-case ${currentPath === "/groups" ? "btn-active" : ""
            }`}
        >
          <UsersIcon className="size-5 text-base-content opacity-70" />
          <span>Groups</span>
